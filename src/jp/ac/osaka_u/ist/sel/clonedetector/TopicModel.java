...
public class TopicModel {
	...
	public void outputBlockList() throws Exception{
		...
		for (Block block : CloneDetector.blockList) {
	...
	public static void calculate(int numTopics) throws Exception {
		...
		PrintWriter output = new PrintWriter (new BufferedWriter(new FileWriter ((new File(CloneDetector.DATASET_FILE)))));
		...
		for (int doc = 0; doc < data.size(); doc++) {
			...
			CloneDetector.blockList.get(doc).setVector(vector);
			CloneDetector.blockList.get(doc).setLen(Math.sqrt(len));
			output.println();
