...
import jp.ac.osaka_u.ist.sel.cmutator.analyze.CAnalyzer4;
import jp.ac.osaka_u.ist.sel.cmutator.data.Block;
import jp.ac.osaka_u.ist.sel.cmutator.data.ClonePair;
import jp.ac.osaka_u.ist.sel.cmutator.data.Method;

public class CMutator {
	...
	private static void commandOption(String[] args) {
		...
		options.addOption(Option.builder("l").longOpt("lang")
				...
				.build());
		options.addOption(
				Option.builder("o").longOpt("outputdir").desc("select directory for result of mutation").hasArg()
						.argName("outputdir").build());
		...
		if (cl.hasOption("outputdir")) {
			Config.OUTPUT_DIR = cl.getOptionValue("outputdir");
		}
		/*
